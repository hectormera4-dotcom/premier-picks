"""
Script de UNA SOLA VEZ para construir el historico de Serie A, igual que
se hizo con LaLiga. Descarga varias temporadas de football-data.co.uk y
las combina en un solo archivo.

Correr esto UNA VEZ desde tu computadora (no es parte del pipeline diario):
    python construir_historico_seriea.py
"""
import pandas as pd
import requests
import io
...
for temporada in TEMPORADAS:
    url = f"https://www.football-data.co.uk/mmz4281/{temporada}/I1.csv"
...
if partes:
    ...
    historico.to_csv("serie_a_combinado.csv", index=False)
    print(f"\nListo: serie_a_combinado.csv creado con {len(historico)} partidos en total.")
else:
